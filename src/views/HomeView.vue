...
<template>
  <div class="container max-w-6xl mx-auto py-6 md:py-8 px-4">
    ...
    <div v-else-if="entries.length === 0"
      class="flex flex-col items-center justify-center min-h-[400px] text-center border-2 border-dashed rounded-2xl bg-muted/20 border-primary/10 transition-colors hover:border-primary/20 p-8">
      ...
      <Card v-for="entry in entries" :key="entry.id"
        ...
        <CardHeader class="space-y-4">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-bold" :class="entry.author_id === currentUser?.id
